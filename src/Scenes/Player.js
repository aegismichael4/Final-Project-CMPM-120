class Player extends Phaser.GameObjects.Sprite {
    ...
    init() {
        ...
        // coyote jump
        this.coyoteTime = 0.15 * 1000; // how long the player can still jump after walking off of a ledge
        this.coyoteTimer = 0;
        this.jumpStartedTimer = false;
        this.startedCoyoteTime = false;

        // walljump
        this.maxSpeedToActivateWallStick = 200; // stops player from wallsticking if they're falling or rising too fast
        this.wallStickTime = 0.05 * 1000; // how long the player actually sticks to a ledge
        ...
        this.wallJumpDisabledLeft = false;
        this.wallJumpDisabledRight = false;
    ...
    initPhysics(groundBTLayer, groundPILayer, door, artifact) {
    ...
    update(delta) {
        ...
        if (this.flipped) {
            ...
            this.scene.particles.updateSlideParticles((this.sliding || (this.diving && this.body.blocked.right)), !this.flipX, this.flipped, this.body.blocked.right);
        } else {
            this.groundMovement();
            this.touchingGround();
            this.jump();
            ...
            this.scene.particles.updateSlideParticles((this.sliding || (this.diving && this.body.blocked.down)), !this.flipX, this.flipped, this.body.blocked.down);
        }

        this.scene.particles.updateRunParticles(this.running, !this.flipX, this.flipped, this.body.velocity.x, this.body.velocity.y);
        this.collisionCheck();
    }
    ...
    groundMovement() {
        ...
        if(this.aKey.isDown) {
            ...
            if (this.body.blocked.down) { // grounded
                ...
                if (!this.wallJumpDisabledRight && !this.usedWallStick &&  Math.abs(this.body.velocity.y) < this.maxSpeedToActivateWallStick && this.body.velocity.x <= 0 && this.tileCheck(this.body.x + 25 + this.maxWallJumpDistance, this.y)) {
            ...
            if (this.wallStickTimer < this.wallStickForgivenessTime && !(this.sliding || this.diving)) {
                ...
                this.body.velocity.x <= 0 ? this.anims.play("run", true) : this.anims.play("turnaround", true);
            ...
            if (this.body.blocked.down) { // grounded
                ...
                // if you havent used wall stick and turn away from a wall while pressed against it
                if (!this.wallJumpDisabledLeft && !this.usedWallStick && Math.abs(this.body.velocity.y) < this.maxSpeedToActivateWallStick && this.body.velocity.x >= 0 && this.tileCheck(this.body.x - 8 - this.maxWallJumpDistance, this.y)) {
                    this.wallStickX = this.x;
                    this.wallStickY = this.y;
                    this.wallStickTimer = this.wallStickTime;
                    this.usedWallStick = true;
    ...
    flippedGroundMovement() {
        ...
        if(this.dKey.isDown) {
            ...
            if (this.body.blocked.right) { // grounded
                ...
                // if you havent used wall stick and turn away from a wall while pressed against it
                if (!this.wallJumpDisabledLeft && !this.usedWallStick &&  Math.abs(this.body.velocity.x) < this.maxSpeedToActivateWallStick && this.body.velocity.y <= 0 && this.tileCheck(this.x, this.y + 14 + this.maxWallJumpDistance)) {
                    this.wallStickX = this.x;
                    this.wallStickY = this.y;
                    this.wallStickTimer = this.wallStickTime;
                    this.usedWallStick = true;
            ...
            if (this.wallStickTimer < 0 && !(this.sliding || this.diving)) {
                ...
                this.body.velocity.y <= 0 ? this.anims.play("run", true) : this.anims.play("turnaround", true);
                this.running = true;
            }
            ...
            if (this.body.blocked.right) { // grounded
                ...
                // if you havent used wall stick and turn away from a wall while pressed against it
                if (!this.wallJumpDisabledRight && !this.usedWallStick && Math.abs(this.body.velocity.x) < this.maxSpeedToActivateWallStick && this.body.velocity.y >= 0 && this.tileCheck(this.x, this.y - 14 - this.maxWallJumpDistance)) {
                    this.wallStickX = this.x;
                    this.wallStickY = this.y;
                    this.wallStickTimer = this.wallStickTime;
                    this.usedWallStick = true;
    ...
    touchingGround() {
        ...
        if(!this.body.blocked.down) {
            ...
            if (this.wallJumpDisabledLeft && !this.tileCheck(this.body.x - 8 - this.maxWallJumpDistance, this.body.y)) this.wallJumpDisabledLeft = false;
            if (this.wallJumpDisabledRight && !this.tileCheck(this.body.x + 25 + this.maxWallJumpDistance, this.body.y)) this.wallJumpDisabledRight = false;
            ...
            if (!this.sliding && !this.diving && this.lastFrameYVelocity > 50) {
                this.scene.thudFX(this.lastFrameYVelocity, this.flipped);
            }
        }
    }
    ...
    flippedTouchingGround() {
        ...
        if(!this.body.blocked.right) {
            ...
            if (this.wallJumpDisabledLeft && !this.tileCheck(this.x, this.y + 14 + this.maxWallJumpDistance)) this.wallJumpDisabledLeft = false;
            if (this.wallJumpDisabledRight && !this.tileCheck(this.x, this.y - 14 - this.maxWallJumpDistance)) this.wallJumpDisabledRight = false;
    ...
    jump() {
        ...
        if(Phaser.Input.Keyboard.JustDown(this.spaceKey)) {
            ...
            if (this.wallStickTimer > this.wallStickForgivenessTime) {
                ...
                this.wallStickTimer = this.wallStickForgivenessTime;
                this.usedWallStick = false;
                ...
                this.scene.particles.wallJumpParticles(Math.sign(this.body.velocity.x) == 1, false);
                ...
                if (!this.tileCheck(this.body.x, this.y - 55) && !this.tileCheck(this.body.x + (Math.sign(this.body.velocity.x) * 36), this.y - 55)) {
                    ...
                    this.scene.particles.jumpParticles(false, this.body.velocity.x, this.body.velocity.y);
                }
                ...
                // player jumps while already touching a wall
                // disable wall jump until player moves away from that wall (see touching ground)
                if (this.tileCheck(this.body.x + 25 + this.maxWallJumpDistance, this.body.y)) {
                    this.wallJumpDisabledRight = true;
                } else if (this.tileCheck(this.body.x - 8 - this.maxWallJumpDistance, this.body.y)) {
                        this.wallJumpDisabledLeft = true;
                }                
            }
        }
    ...
    flippedJump() {
        ...
        if(Phaser.Input.Keyboard.JustDown(this.spaceKey)) {
            ...
            if (this.wallStickTimer > this.wallStickForgivenessTime) {
                ...
                this.scene.wallJump.play();
                this.scene.particles.wallJumpParticles(Math.sign(this.body.velocity.y) == 1, true);
                ...
                if (this.tileCheck(this.x, this.y + 14 + this.maxWallJumpDistance)) {
                    this.wallJumpDisabledRight = true;
                } else if (this.tileCheck(this.x, this.y - 14 - this.maxWallJumpDistance)) {
                        this.wallJumpDisabledLeft = true;
                }
            }
    ...
    slide(delta) {
        ...
        if (Phaser.Input.Keyboard.JustDown(this.sKey) && !this.sliding && this.slideCooldownTimer <= 0) {
            ...
            // hard code fix for player sliding off the map on the left
            if (this.x < 40) return;
    ...
    touchedObstacle(shatter) {
        this.setVisible(false);
        this.scene.killPlayer(shatter, this.body.velocity.x, this.body.velocity.y);
    }
    ...
    collisionCheck() {
        ...
        if (this.tall) {
            ...
            if (tile) this.touchedObstacle(tile.properties.shatter == true);
